...
// via https://github.com/3D4Medical/glTFSceneKit/blob/59b099b28730e30394774330af90ebc84418fa68/Sources/glTFSceneKit/GLTF/JSONCodingKeys.swift
    ...
    public mutating func encode(_ value: Dictionary<String, Any>) throws {
        try value.forEach({ (key, value) in
            ...
            case let value as Date:
                try encode(Int(value.timeIntervalSince1970 * 1000), forKey: key)
    ...
    public mutating func encode(_ value: Array<Any>) throws {
        try value.enumerated().forEach({ (index, value) in
            ...
            case let value as Double:
                try encode(value)
            case let value as Date:
                try encode(Int(value.timeIntervalSince1970 * 1000))
            case let value as CGFloat:
                try encode(value)
            case let value as Dictionary<String, Any>:
                try encode(value)
